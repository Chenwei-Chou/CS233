...
for:
    bge $s0 2 return
    li  $t0 0
    sb  $t0 puzzle_received  # puzzle_received = 0;
    la  $t0 board           # *REQUEST_PUZZLE = &board;
    sw  $t0 REQUEST_PUZZLE  # *REQUEST_PUZZLE = &board;

    while:                  # while (puzzle_received == 0) {}
    ...
    jal quant_solve         # quant_solve(board);
    
    la  $t0 board           # *SUBMIT_SOLUTION = &board;
    sw  $t0 SUBMIT_SOLUTION # *SUBMIT_SOLUTION = &board;  // Send the solution location
...
request_puzzle_interrupt:
    ...
    ## added
    li $t0 1
    sw $t0 REQUEST_PUZZLE_ACK   # *REQUEST_PUZZLE_ACK = (int)1; // ACK the interrupt
    sb $t0 puzzle_received      # puzzle_received = 1;
    ## added
